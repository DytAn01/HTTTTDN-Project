...
public class daothongke {
    ...
    public ArrayList<ThongKeSanPhamDTO> getThongKeSanPham(java.sql.Date fromDate, java.sql.Date toDate) throws SQLException {
        String sql = "SELECT "
                ...
                + "IFNULL(sales.soLuongBan, 0) AS soLuongBan, "
                + "IFNULL(sales.doanhThu, 0) AS doanhThu, "
                + "IFNULL(cost.chiPhi, 0) AS chiPhi, "
                + "(IFNULL(sales.doanhThu, 0) - IFNULL(cost.chiPhi, 0)) AS loiNhuan "
                + "FROM sanpham s "
                + "LEFT JOIN ("
                + "    SELECT cth.maSanPham, SUM(cth.soLuong) AS soLuongBan, SUM(cth.soLuong * cth.donGia) AS doanhThu "
                + "    FROM hoadon h "
                + "    JOIN chitiethoadon cth ON h.maHoaDon = cth.maHoaDon "
                + "    WHERE h.isDelete = 0 AND DATE(h.ngayMua) BETWEEN ? AND ? "
                + "    GROUP BY cth.maSanPham"
                + ") AS sales ON s.maSanPham = sales.maSanPham "
                + "LEFT JOIN ("
                + "    SELECT ctpn.maSanPham, SUM(ctpn.soLuong * ctpn.giaNhap) AS chiPhi "
                + "    FROM phieunhap p "
                + "    JOIN chitietphieunhap ctpn ON p.maPhieuNhap = ctpn.maPhieuNhap "
                + "    WHERE p.isDelete = 0 AND DATE(p.ngayNhap) BETWEEN ? AND ? "
                + "    GROUP BY ctpn.maSanPham"
                + ") AS cost ON s.maSanPham = cost.maSanPham "
        ...
        try (Connection con = connect.connection();
            ...
            ResultSet rs = stmt.executeQuery();
            ArrayList<ThongKeSanPhamDTO> resultList = new ArrayList<>();
            while (rs.next()) {
                int maSanPham = rs.getInt("maSanPham");
                String tenSanPham = rs.getString("tenSanPham");
                long soLuongBan = rs.getLong("soLuongBan");
                long doanhThu = rs.getLong("doanhThu");
                long chiPhi = rs.getLong("chiPhi");
                long loiNhuan = rs.getLong("loiNhuan");
                resultList.add(new ThongKeSanPhamDTO(maSanPham, tenSanPham, soLuongBan, doanhThu, chiPhi, loiNhuan));
            }
            return resultList;
        }
    }
